...
export default function ChatInput({ setBanner }) {
  ...
  const { isLoading, setIsLoading, addMessage, setArtifacts } = useMessage();
  ...
  return (
    <div className="w-full overflow-hidden px-3 md:px-5 py-4 border-t border-white/6 bg-[#0d0f14]">
      <div className="flex flex-col gap-2 bg-white/3 border border-white/[0.07] rounded-3xl px-4 pt-3.5 pb-3">
        {/* Agent selector */}
          ...
          {agents.map((agent) => {
            ...
            return (
              <button
                ...
                className={`shrink-0 inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-medium border transition-all ${
                  isActive
            ...
            <div className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/4 px-3 py-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500">
                  {Math.ceil(selectedFile.size / 1024) < 1000
